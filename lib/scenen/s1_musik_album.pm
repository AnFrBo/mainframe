...
# #########################################################################
#
# die Musik Auswahl Album Scene       
#
# Joachim Bothe 14.7.2012
#
# #########################################################################

package s1_musik_album;
...
sub init {
    ...
    tool::mfLog( 'trace', "s1_musik_album: Interpret: $interpret_nr" );
    ...
    $max_zeile = $master{$interpret_nr}{'MAX_ALBUM'};
    tool::mfLog( 'trace', "s1_musik_album: $max_zeile" );
...
sub getInfo {
    ...
    my $ret = sprintf( "%d;%d", $interpret_nr, ( $akt_pla_zeile + $akt_txt_zeile ));
    return( $ret );
}
    ...
    if ($action == GLUT_KEY_UP) {
        ...
        if( $akt_txt_zeile < 0 ) {
            if( $max_zeile_flag ) {
                $akt_pla_zeile--;
                if( $akt_pla_zeile < 0 ) {
                    $akt_pla_zeile = $master{$interpret_nr}{'MAX_ALBUM'};
                }
                $akt_txt_zeile = 0;
            } else {
        ...
        if( $akt_txt_zeile > $max_zeile ) {
            if( $max_zeile_flag ) {
                $akt_pla_zeile++;
                if( $akt_pla_zeile > $master{$interpret_nr}{'MAX_ALBUM'} ) {
                    $akt_pla_zeile = 0;
                }
                $akt_txt_zeile = $max_zeile;
            } else {
...
sub movText {
    ...
    foreach (0..$max_zeile) {
        ...
        $pl_zeile = $_ + $akt_pla_zeile;

        if( $pl_zeile > $master{$interpret_nr}{'MAX_ALBUM'}) {
            $pl_zeile -= $master{$interpret_nr}{'MAX_ALBUM'} +1;
        }
        if( $pl_zeile < 0) {
            $pl_zeile += $master{$interpret_nr}{'MAX_ALBUM'} -1;
        }
        $zeile_text[$_] = $master{$interpret_nr}{$pl_zeile}{'ALBUM'};
        $zeile_akt_x[$_]  = (text::width3dText($zeile_text[$_]) / 2.0) * (-1);
    }
}
