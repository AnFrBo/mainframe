...
# #########################################################################
#
# die Musik Auswahl Interpreter Scene       
#
# Joachim Bothe 14.7.2012
#
# #########################################################################

package s1_musik_interpret;
...
sub getInfo {
    ...
    my $ret = sprintf("%d;", $akt_pl_zeile + $akt_txt_zeile);
    tool::mfLog( 'trace', "s1_musik_interpret: $ret" );
...
sub setAction {
    ...
    if ($action == GLUT_KEY_UP) {
        ...
        if( $akt_txt_zeile < 0 ) {
            $akt_pl_zeile--;
            if( $akt_pl_zeile < 0 ) {
                $akt_pl_zeile = $master{'MAX_INTERPRET'};
            }
            $akt_txt_zeile = 0;
        }
        &berechneZ();
        ...
        if( $akt_txt_zeile > MAX_ZEILE ) {
            $akt_pl_zeile++;
            if( $akt_pl_zeile > $master{'MAX_INTERPRET'} ) {
                $akt_pl_zeile = 0;
            }
...
sub movText {
    ...
    foreach (0..MAX_ZEILE) {
        ...
        $pl_zeile = $_ + $akt_pl_zeile;
        if( $pl_zeile > $master{'MAX_INTERPRET'}) {
            $pl_zeile -= $master{'MAX_INTERPRET'} +1;
        }
        if( $pl_zeile < 0) {
            $pl_zeile += $master{'MAX_INTERPRET'} -1;
        }
        $zeile_text[$_] = $master{$pl_zeile}{'INTERPRET'};
        $zeile_akt_x[$_]  = (text::width3dText($zeile_text[$_]) / 2.0) * (-1);
    }
}
